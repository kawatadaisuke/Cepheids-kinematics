...
Tvxvyvz = bovy_coords.vrpmllpmbb_to_vxvyvz(
    hrvv, pmlonv, pmlatv, glonv, glatv, distv, degree=True)
vxv = Tvxvyvz[:, 0] + usun
vyv = Tvxvyvz[:, 1] + vsun
vzv = Tvxvyvz[:, 2] + wsun
...
# computing correlation coefficients with logP
dangarmlim=2.5
darmlim=1.5
pdangarmccoef = np.zeros(nmc)
pdarmccoef = np.zeros(nmc)
logpvtile=np.tile(logpv,(nmc,1))
sindxdangarm=np.where(np.abs(dangdiffarmv_mean)<dangarmlim)
sindxdarm=np.where(np.abs(darmv_mean)<darmlim)
for i in range(nmc):
    pdangarmccoef[i] = np.corrcoef(dangdiffarmv_sam[i,sindxdangarm], logpvtile[i, sindxdangarm])[0, 1]
    pdarmccoef[i] = np.corrcoef(darmv_sam[i,sindxdarm], logpvtile[i, sindxdarm])[0, 1]

print ' dang-logp corrcoef med,mean,sig,N=', np.median(pdangarmccoef), \
np.mean(pdangarmccoef), np.std(pdangarmccoef),np.size(sindxdangarm)
print ' d-logp corrcoef med,mean,sig=', np.median(pdarmccoef), \
np.mean(pdarmccoef), np.std(pdarmccoef),np.size(sindxdarm)

# plot dang vs. log P
plt.subplot(1, 2, 1)
plt.errorbar(dangdiffarmv_mean, logpv,
             xerr=dangdiffarmv_std, fmt='.')
plt.xlabel(r"d$_{\theta}$ (kpc)", fontsize=12, fontname="serif")
plt.ylabel(r"Log P", fontsize=12, fontname="serif")
plt.axis([-dangarmlim, dangarmlim, 0.2, 2.0])
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
# plot darm vs. Log P
plt.subplot(1, 2, 2)
plt.errorbar(darmv_mean, logpv,
             xerr=np.std(darmv_sam, axis=0).reshape(nstarv), fmt='.',)
plt.xlabel(r"d$_{arm}$ (kpc)", fontsize=12, fontname="serif")
plt.ylabel(r"Log P", fontsize=12, fontname="serif")
plt.axis([-darmlim, darmlim, -0.2, 2.0])
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
plt.show()
    ...
    for i in range(nmc):
        if MCsample_v == True:
            print >>f, "%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f" % ( \
                ...
                vradv_mean[j], vrotv_mean[j], rsun_sam[i, j], vcircrv_sam[i, j], \
                usun_sam[i, j], rgalv_sam[i, j], rref_sam[i, j], angref_sam[i, j], \
                dangdiffarmv_sam[i,j],dangdiffarmv_mean[j],logpv[j], \
                np.sqrt(xposv_sam[i,j]**2+yposv_sam[i,j]**2))
        else:
            print >>f, "%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f" % (
                xposv_sam[i, j], yposv_sam[i, j], xposv[j], yposv[j], vradv_sam[i, j], \
                vrotv_sam[i, j], vradv[j], vrotv[j], darmv_sam[i, j], darmv_mean[j], \
                vradv_mean[j], vrotv_mean[j],dangdiffarmv_sam[i,j],logpv[j], \
                np.sqrt(xposv_sam[i,j]**2+yposv_sam[i,j]**2))
f.close()
